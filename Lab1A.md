...
I found my private IP address and verified that the network was set up properly by using the ip a command. I used Google's DNS server (8.8.8.8) and ping to verify internet connectivity. I then added a custom hostname alias to the /etc/hosts file and used the ping command to confirm that it functioned. Next, I investigated the nslookup and whois commands to obtain domain information and see the relationship between IP addresses and domain names. Finally, I found my public ip address from https://www.whatismyip.com/
